...
// than seamless-immutable directly. On EB 1.21 (pnpm) Visual Studio resolves
// seamless-immutable's index.d.ts as a non-module and errors (TS2306), while
// jimu-core always resolves through the widget tsconfig path mapping. This is
// a type-only import (erased at build), so the runtime bundle is unchanged.
...
export interface LegendEl extends ElBase {
  type: 'legend'
  maxItems: number
  /** Column count authored in the .pagx (ManualColumns); 0/undefined = auto. */
  columns?: number
  /** Title visibility authored in the .pagx; undefined = fitter default. */
  showTitle?: boolean
}
...
export interface Config {
  ...
  /** Write export diagnostics to the browser console (off by default).
   *  Reports what the selected-feature sweep found and how much of it
   *  reached the page, which is the only way to tell why a highlight did
   *  not print, since the selection lives outside this widget. */
  diagnostics?: boolean
  /** Selection highlight color on the print, as a #rrggbb hex string.
   *  Defaults to the SDK's cyan so the print matches the screen. */
